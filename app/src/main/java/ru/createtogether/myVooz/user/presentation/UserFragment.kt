...
import ru.createtogether.myVooz.databinding.FragmentUserBinding
...
class UserFragment : Fragment() {
    ...
    private fun initData(authUserModel: AuthUserModel?) {
        authUserModel?.apply {
            ...
            if(idGroupOfUser == 0){
                ...
                groupOfUser?.let {
                    binding.tvGroupName.text = it.nameGroup
                }
            }
        }
    }
    ...
    private fun setChangeUserNameButton() {
        binding.clChangeUserNamebutton.setOnClickListener {
            val changeFullNameDialogFragment = ChangeFullNameDialogFragment()
            changeFullNameDialogFragment.show(childFragmentManager, ChangeFullNameDialogFragment::class.java.simpleName)
        }
    }

    private var photoPickerResult =
    ...
    private fun addBackButton() {
        binding.toolbar.navigationIcon =
            ContextCompat.getDrawable(requireContext(), R.drawable.ic_arrow_left)
